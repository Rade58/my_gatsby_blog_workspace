...
DAKLE IGNORISAO SAM GORNJI PROBLEM, JER SAM VIDEO I NEKE OZBILJNIJE DOCUMENTATIO NSITE-OVE KAKO SU UPRAVO OVAJ PROBLEM CAK I RUZNIJE DEFINISALI

# SLEDECE STA MOGU DEFINISATI JESTE DODAVANJE FUNKCIONALNOSTI ZA OCTICON ARROWS, JER KAO STO ZNAS ONE BI TREBALE DA SLUZE ZA `'PRESKAKANJE'`, NA SLEDECI, ODNONO PREDHODNI HEADING

U SUSTINI TREBAM DA URADIM SLEDECE

- WRAPP-OVANJE OCTICON-A U LINK-OVE

- ALI DINAMICKI TREBA DA SE MENJAJU HREF-OVI TIH LINKOVA (ODENONO U OVOM SLUCAJU JA UMESTO `href` KORISTIM `to` ATRIBUT NA `Link` KOMPONENTI)

- DOSTUPAN MI JE NIZ `headings`, IMAM I STATE, KOJI CUVA id HEDING DIV-A, KOJI JE TRENUTNO U VIEWPORT-U, NA STRANICI (U PITANJU JE OVAJ STATE: `intersectedDivId`)

- TREBAO BI UZ POMOC `indexOf` METODE PRONACI, O KOJEM SE TO INDEKSU RADI, **I ONDA NA OSNOVU TOGA PRONADJEM `INDEKS PREDHODNOG CLANA` ODNONO `IDEKS SLEDECEG CLANA`**

- I SADA KADA IMAM TA DVA POMENUTA IDEKS, MOGU DA ACCESS-UJEM PREDHODNOM, ODNONO SLEDECEM HEADING-U

- I UPRAVO KORISTIM TAJ PREDHODNI ODNONO SLEDECI HEADING DA BI FORMIRAO VREDNOSTI `to` ATRIBUTA NA ONIM `Link` KOMPONENTAMA, SA KOJIMA SAM WRAPPE-OVAO OCTICONSE 

NAKON NEKIG BUGG-OVA I OVO SAM SVE DEFINISAO

MOZDA CU OVO JEDNOM PRILIKOM REVISIT-OVATI DA VIDIM DA LI JE SVE U REDU

DO DUSE NEKADA SE DESAVAJU GRESKE U SLUCAJU KADA SE DVA HEADING-A MOGU NACI ZAJEDNO U VIEWPORT-U

## ISTO TAKO VODI RACUNA DA UOPSTE NEMAS rootMargin ZA INTERSECTION OBSERVER

# POSTOJI I ONA ARROW-UP STRELICA, A DEFINISACU DA SE PRITISKOM NA NJU SCROLL-UJE TO THE TOP OF THE PAGE

TO SAM DEFINISAO

# SADA JE POTREBNO DEFINISATI DA JUMPER ELEMENT BUDE FIXNO POZICIONIRAN ZA SIRINE EKRANE MANJE OD `918px`

NARAVNO ON MALO MORA PROEMNITI OBLIK U TO M STANJU, I IAMTI MALO MANI RASPOTRED

ISTO TAKO MORAJU SE SAKRITI NEKE STVARI

TO SAM URADIO

# ZELIM DA NA MOBILE EKRANIMA JUMPER KOMPONENTA BUDE SAKRIVENA DESNO, PA KADA KORISNIK SCROLL-UJE UP, DA SE ONA SLID-UJE INTO VIEW

NAKON SOME THINKING AND TWEAKING AROUND DOSAO SAM NA IDEJU DA URADIM NESTO STA SAM VEC URADIO RANIJE

SECAS SE KAKO SAM VEC JEDNOM DISPATCH-OVAO KAKO BI U PAYLOAD-U PROSLEDIO SET STATE FUNKCIJU, KOJA, DA BI ONA BILA ONDA U DRUGOJ KOMPONENTI DOSTUPNA KROZ REDUCER STATE

TO CU URADITI I SADA

POSTO VEC IMAM HEADER KOJI SE SLIDE-UJE DOLE KADE SE SCROLL-UJE UPA, I SLID-UJE GORE KADA SE SCROLL-UJE DOWN, JA CU SET STATE FUNKCIJU IZ `packages/gatsby-theme-raedal/src/components/jumper-h.tsx` DISPATCH-OVATI

U JUMPER-U DISPETCH-OVACU ACTION TYPE, I U PAYLOADU CU POSLATI FUNKCIJU, A VAZNO DA NAPOMENEM DA CU TO URADITI ON MOUNTING (useEffect I TU KORISTIM)

**A NA TAJ NACIN ONA CE BITI DOSTUPNA U HEADER-U I U HEADER-U JE MOGU POZVATI U ODNOSU NA TO KOJA SLIDING UP/SLIDING DOWN KLASA SE KORISTI TAMO**

DA NE DAJEM VISE PREVISE IZLISNIH KOMENTARA VIDECES I SAM STA SAM URADIO

NARAVNO OPET MORAM 'PROSIRIVATI REDUCER-OV STATE'

## `Cannot update a component ('JumperButtons') while rendering a different component`

OVAKAV WARNING SI MOGAO DOBITI AKO SI POZVAO SET STATE JEDNE KOMPONENTE U DRUGOJ, PRI NJENO MRENDERING-U

OVAJ ERROR CE NESTATI AKO POZOVES SET STATE U useEffect-U

JA MOGU DEFINISATI `useEffect`, KOJI CE SE ZVATI ON EVARY UPDATE OF `packages/gatsby-theme-raedal/src/components/header.tsx`

I U NJEMU MOGU ZVATI POMENUTI SET STATE

`useEffect` KOJI SE POZIVA ON EVERY UPDATE, JESTE ONAJ, KOJI NEMA ZADANT DEPENDANCY ARRAY ARGUMENT

I TO SAM USPESNO DEFINISAO
