	...
	allFile(filter: {name: {regex: "/graphql_interfaces/"}}){  # OVO JE DEO IMENA IMAGE FAJLOVA ZA JEDAN PAGE (KORISTIM REGEXP)
        ...
        childCloudinaryAsset {
          fluid {
            ...
            # DA TI NAPOMENEM DA TI OVA  FRAGMENT NIJE DOSTUPAN U GRAPHI-JU ALI BICE
            # TI DOSTUPAN U CODEBASE-U

            ...CloudinaryAssetFluid

            # TAKO DA SAM JA UMESTO OVOGA KORISTIO POJEDINACNE FIELD-OVE (ALI SE ISPOSTAVILO KASNIJE DA FRAGMENT NE MOGU KORISTITI
            # U HOOK-U    createPages   (A JA CU U NODE FAJLU PRAVITI QUERY) )
...
# :eight: SADA MOGU DA PROSLEDIM STVARI KROZ KONTEXT (REACT CONTEXT), ZA BLOG POST PAGE

A TU MORAM I DA TYPE-UJEM `packages/gatsby-theme-raedal/src/templates/blog-post-template.tsx`

ALI `packages/gatsby-theme-raedal/src/context_n_reducers/context_n_reducer_blog_post.ts`, KAO STO SAM I REKAO

SVE SAM TO ODRADIO

OSTJE MI SADA DE DEFINISEM KOMPONENTE KOJE CE KORISTITI TAJ DATA

ALI SLIKE TREBAJU DA BUDU DEO KOJI JE OUTPUTED BY `MDXRenderer`
...
# DAKLE SADA IMAM DATA, ALI MORAM DA VIDIM KAKO TO, DA JA 'BELEZIM' ILI MARK-UJEM IMAGE-OVE U, SAMOM MDX-U, KAO IMAGE-OVE PLANIRANE DA SE TU RENDER-UJU IMAGE-OVI TRANSFORMED BY CLOUDINARY

SECAS SE MESTA GDE SI UPOTREBIO `MDXProvider`

TO JE BILO OVDE:

`packages/gatsby-theme-raedal/src/components/article.tsx`

**I TADA SI PROSLEDJIVAO, TVOJE CUSTOM KOMPONENTE**
...
MISLIM DA SAM ZA SADA ZAVRSIO SA DEVELOPINGOM VEZANIM ZA CLOUDINARY IMAGES

TAKO DA CU PRECI U SLEDECI BRANCH, U KOJEM CU MALO MODIFIKOVATI SHADOWS ACROSS DIFFRENT VRSTA STRANICA